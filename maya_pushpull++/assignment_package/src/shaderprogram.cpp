...
ShaderProgram::ShaderProgram(GLWidget277 *context)
    : vertShader(), fragShader(), prog(),
      ...
      attrJIndex(-1), attrJWeight(-1),
      unifModel(-1), unifModelInvTr(-1), unifViewProj(-1), unifColor(-1),
      unifBindMA(-1), unifTransformMA(-1),
      context(context)
{}
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    attrJIndex = context->glGetAttribLocation(prog, "vs_JIndex");
    attrJWeight = context->glGetAttribLocation(prog, "vs_JWeight");
    ...
    unifBindMA = context->glGetUniformLocation(prog, "u_BindMA");
    unifTransformMA = context->glGetUniformLocation(prog, "u_TransformMA");
}
...
void ShaderProgram::setBindMatrixArray(const std::vector<glm::mat4> &bindMA)
{
    useMe();

    if(unifBindMA != -1)
    {
        context->glUniformMatrix4fv(unifBindMA,
                                    (GLsizei)bindMA.size(),
                                    GL_FALSE,
                                    &bindMA[0][0][0]);
    }
...
void ShaderProgram::setTransformMatrixArray(const std::vector<glm::mat4> &transformMA)
{
    useMe();

    if(unifTransformMA != -1)
    {
        context->glUniformMatrix4fv(unifTransformMA,
                                    (GLsizei)transformMA.size(),
                                    GL_FALSE,
                                    &transformMA[0][0][0]);
    }
}
...
void ShaderProgram::draw(Drawable &d)
{
    ...
    if (attrJIndex != -1 && d.bindJIndex()){
        context->glEnableVertexAttribArray(attrJIndex);
        context->glVertexAttribIPointer(attrJIndex, 4, GL_UNSIGNED_INT, 0, NULL);
    }

    if (attrJWeight != -1 && d.bindJWeight()){
        context->glEnableVertexAttribArray(attrJWeight);
        context->glVertexAttribPointer(attrJWeight, 4, GL_FLOAT, false, 0, NULL);
    }
    ...
    if (attrJIndex != -1) context->glDisableVertexAttribArray(attrJIndex);
    if (attrJWeight != -1) context->glDisableVertexAttribArray(attrJWeight);
